...
RSpec.describe "LLM::Chat: llamacpp" do
  ...
  context "when given an instance of LLM::Function::Return",
          vcr: {cassette_name: "llamacpp/conversations/system_function_call_1"} do
    let(:params) { {tools: [tool]} }

    before do
      bot.chat("/nothink", role: :system)
  ...
  context "when given an instance of LLM::Function::Return (via an array)",
          vcr: {cassette_name: "llamacpp/conversations/system_function_call_2"} do
    let(:params) { {tools: [tool]} }
